...
int predict(){ 
 char hiden[]="password";
 char guess[20];
 char str[5]; // Add declaration for `str`
...
int predict(){ 
 ...
 snprintf(str,sizeof(str), "%s", guess); // Prevent overflows by enforcing a maximum size for `str` buffer
 return strcmp(guess,hidden);
} 
